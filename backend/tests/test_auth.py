...
def test_change_password(client, admin_user):
    ...
    assert client.post("/api/v1/auth/login", json={"email": "admin@test.io", "password": "Password123!"}).status_code == 401
    assert client.post("/api/v1/auth/login", json={"email": "admin@test.io", "password": "NewPassword456!"}).status_code == 200
